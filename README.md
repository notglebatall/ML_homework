...
2. **Feature Engineering работает**, но требует глубокого понимания предметной области и экспериментов

3. **Регуляризация (особенно Lasso) полезна** при большом количестве признаков для отбора наиболее значимых

4. **Простые методы часто эффективны**: заполнение пропусков медианой, стандартизация, базовое OHE-кодирование дали хорошие результаты

5. **Время - ограничивающий фактор**: многие идеи остались нереализованными из-за дедлайна

6. **Важность EDA**: визуализации помогли выявить нелинейные зависимости (например, квадратичная зависимость от года)

7. **GridSearchCV - must have** для подбора гиперпараметров, но требует вычислительных ресурсов
...
- Полиномиальные признаки с последующим отбором
- Ансамблирование нескольких моделей
- Более сложные методы импутации пропусков
